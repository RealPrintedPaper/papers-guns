...
{
	public class PancorJackhammerP : ModProjectile
	{
		public override string Texture => "PapersGuns/Projectiles/Arsenal/Textures/GoldenPancorJackhammer";
		...
		public override void AI()
		{
			...
			if (spawnin == true)
			{
				CombatText.NewText(Main.player[Projectile.owner].getRect(), Color.Red, "PANCOR JACKHAMMER!", false, false);
				spawnin = false;
			}
			...
			if (Projectile.ai[0] % 13f == 1f)
			{
				SoundEngine.PlaySound(SoundID.Item38, Main.player[Projectile.owner].position);
		...
		public override void OnKill(int timeLeft)
		{
			...
			Gore.NewGore(source, new Vector2(Projectile.position.X + (float)(Projectile.width / 2) - 24f, Projectile.position.Y + (float)(Projectile.height / 2) - 24f), new Vector2(Main.rand.NextFloat(-2, 2), Main.rand.NextFloat(-5, -2)), ModContent.GoreType<Gores.PancorJackhammerGore>(), 1f);
		}
	}
}