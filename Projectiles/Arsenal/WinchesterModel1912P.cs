...
{
	public class WinchesterModel1912P : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (spawnin == true)
			{
				CombatText.NewText(Main.player[Projectile.owner].getRect(), Color.Red, "WINCHESTER MODEL 1912!", false, false);
				spawnin = false;
			}
			...
			if (Projectile.ai[0] % 30f == 1f)
			{
				SoundEngine.PlaySound(SoundID.Item36, Main.player[Projectile.owner].position);

				for (int i = 0; i < 5 + Main.rand.Next(2); i++)
				{
		...
		public override void OnKill(int timeLeft)
		{
			...
			Gore.NewGore(source, new Vector2(Projectile.position.X + (float)(Projectile.width / 2) - 24f, Projectile.position.Y + (float)(Projectile.height / 2) - 24f), new Vector2(Main.rand.NextFloat(-2, 2), Main.rand.NextFloat(-5, -2)), ModContent.GoreType<Gores.WinchesterModel1912Gore>(), 1f);
		}
	}
}